...
class APIService: ObservableObject {
    @Published var response = [People]()
    @Published var planetResponse = [Planet]()
    ...
    // Load the star wars people form the swapi
    func loadPeople() -> AnyPublisher<[People], Error> {
        ...
        let publisher = URLSession.shared.dataTaskPublisher(for: url!)
            ...
            .map{ $0.results } // Map PeopleResponse results
            .mapError { error -> Error in
    ...
    func loadPlanet(_ id: String) -> AnyPublisher<[Planet], Error> {
        ...
        self.publisherRequest = publisher.receive(on: DispatchQueue.main)
            .sink(receiveCompletion: { value in
                ...
                self.planetResponse = data; // Set the array of planets to the response
            })
